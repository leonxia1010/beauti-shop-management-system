...
- [ ] Create domain models and Zod schemas for revenue and cost data (AC: 1, 2, 3, 6)
  - [ ] Define `ServiceSession` model with store_id, beautician_id, service_date, gross_revenue, payment_method fields
  - [ ] Define `CostEntry` model with store_id, category, payer, allocation_rule_id fields
  - [ ] Create Zod validation schemas for both models in libs/domain
  - [ ] Add TypeScript types and export from libs/domain barrel
- [ ] Implement Prisma database schema and migrations (AC: 1, 2, 3, 6)
  - [ ] Create service_sessions table based on data architecture specifications
  - [ ] Create cost_entries table with proper foreign key relationships
  - [ ] Add audit_logs table for tracking data changes
  - [ ] Create and run initial database migration
- [ ] Build backend API endpoints for revenue management (AC: 1, 3, 6)
  - [ ] POST /api/v1/revenue/bulk-import endpoint with file upload handling
  - [ ] GET /api/v1/revenue/sessions endpoint with pagination and filtering
  - [ ] POST /api/v1/revenue/sessions endpoint for single record creation
  - [ ] PUT /api/v1/revenue/sessions/:id endpoint for record updates
  - [ ] Implement business rule engine for beautician share calculation (4/6 split)
...
- [ ] Build admin web UI for revenue data management (AC: 1, 3)
  - [ ] Create bulk import wizard using 3-step Stepper Form (上传 → 预览 → 结果)
  ...
  - [ ] Add Status Tabs for data state classification [全部|🟢 正常|⚠️ 异常|✅ 已确认]
